...
async function handleReservation(request, env) {
  ...
  if (data.error) return json({ message: data.error }, 400);

  const reference = makeReference(data.date);
  const datePt = data.date.split('-').reverse().join('/');
  const timePt = data.time.replace(':', 'h');
  const notes = data.notes || '—';
  const subject = `Pedido de reserva — ${datePt.slice(0, 5)} — ${timePt} — ${data.people} pessoas — ${data.name}`;
  const restaurantText = `NOVO PEDIDO DE RESERVA\n\nData: ${datePt}\nHora: ${timePt}\nPessoas: ${data.people}\nNome: ${data.name}\nTelefone: ${data.contact}\nEmail: ${data.email}\nNotas: ${notes}\nIdioma do cliente: ${data.language}\nReferência: ${reference}\n\nEste pedido ainda não foi confirmado.`;
  const restaurantHtml = `<h2>NOVO PEDIDO DE RESERVA</h2><p><b>Data:</b> ${escapeHtml(datePt)}<br><b>Hora:</b> ${escapeHtml(timePt)}<br><b>Pessoas:</b> ${data.people}<br><b>Nome:</b> ${escapeHtml(data.name)}<br><b>Telefone:</b> ${escapeHtml(data.contact)}<br><b>Email:</b> <a href="mailto:${escapeHtml(data.email)}">${escapeHtml(data.email)}</a><br><b>Notas:</b> ${escapeHtml(notes)}<br><b>Idioma do cliente:</b> ${escapeHtml(data.language)}<br><b>Referência:</b> ${escapeHtml(reference)}</p><p><b>Este pedido ainda não foi confirmado.</b></p>`;
  const receiptText = `Recebemos o seu pedido de reserva no Santa Clara dos Cogumelos.\n\nData: ${datePt}\nHora: ${timePt}\nPessoas: ${data.people}\nNome: ${data.name}\nTelefone: ${data.contact}\nEmail: ${data.email}\nNotas: ${notes}\nReferência: ${reference}\n\nATENÇÃO: este email confirma apenas a receção do pedido. A reserva será válida depois de receber a nossa confirmação.\n\nSanta Clara dos Cogumelos\n+351 913 043 302`;
  const receiptHtml = `<h2>Recebemos o seu pedido de reserva</h2><p><b>Data:</b> ${escapeHtml(datePt)}<br><b>Hora:</b> ${escapeHtml(timePt)}<br><b>Pessoas:</b> ${data.people}<br><b>Nome:</b> ${escapeHtml(data.name)}<br><b>Telefone:</b> ${escapeHtml(data.contact)}<br><b>Email:</b> ${escapeHtml(data.email)}<br><b>Notas:</b> ${escapeHtml(notes)}<br><b>Referência:</b> ${escapeHtml(reference)}</p><p><b>ATENÇÃO: este email confirma apenas a receção do pedido. A reserva será válida depois de receber a nossa confirmação.</b></p><p>Santa Clara dos Cogumelos<br>+351 913 043 302</p>`;
  ...
  await sendEmail(env.BREVO_API_KEY, {
    ...
    subject: `Recebemos o seu pedido de reserva — ${reference}`,
